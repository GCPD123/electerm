...
- Ask 不调用 Agent 或终端写入；本版本仍未增加 embedding、向量数据库或无人值守自动执行能力。
- RAG-M2 已实现：Agent 在请求模型规划前会先执行只读的 `search_fiberhome_knowledge` 预检索，并在工具卡片中展示结果。已连接设备时，可靠依据对应的低风险只读查询可自动执行；配置或其他改动性命令不会自动执行。
- Windows 开发环境已完成真实 XLSX 的手工导入、Ask 引用和 Agent 知识检索验证。项目构建和针对性单测已通过；真实设备的低风险自动执行烟测待设备可用时完成。
...
- Agent 展示检索过程和引用；Agent 模式本身授权可靠依据对应的只读查询自动执行，配置或其他改动性命令仍走独立审核入口。
...
### RAG-M4：巡检与故障诊断知识库（后续版本）

- 保留当前 Excel 命令手册作为命令依据；新增 PDF、DOCX、Markdown、TXT 的本地导入，并保留页码、标题层级和来源位置。
- 巡检手册的核心知识单元是“告警/现象 → 根因 → 判断依据 → 处理步骤 → 风险与回退”，而不是重复存储通用网络协议教材。
- 可导入脱敏后的典型回显、告警样本和判读规则；禁止把真实设备名称、IP、账号、密码、密钥或完整敏感日志写入知识库。
- 文档规模扩大后，将当前 JSON 知识存储迁移到本地 SQLite 全文检索，并提供后台分段导入、进度、取消、失败重试和索引版本迁移。
- 后续再增加可配置的混合/语义检索；若使用云端 embedding，必须在索引前明确告知哪些文档片段会离开本机。
