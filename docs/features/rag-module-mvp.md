...
- RAG-M2 已实现第一步：Agent 在请求模型规划前会先执行只读的 `search_fiberhome_knowledge` 预检索，并在工具卡片中展示结果；知识检索本身不执行终端命令，终端工具仍沿用既有路径。
- Windows 开发环境已完成真实 XLSX 的手工导入验证；仍需完成 Ask 真实问答与引用展示的手工烟测。项目构建和针对性单测已通过。
