...
- **P0（开工门槛）**：统一执行入口；所有入口都必须经过相同策略；授权档位只能改变策略结果，不能关闭网关。默认逐条批准，并允许用户显式选择受范围约束的“替我审批”或“受控完全执行”。
...
- 所有执行都必须记录。默认“请求批准”；用户显式选择“替我审批”后，可对明确低风险、只读、单目标命令自动放行；“受控完全执行”也必须受会话、目标和硬阻断约束。
...
| 等级 | 示例 | 默认决策 |
|---|---|---|
| 低 | `pwd`、`whoami`、`df -h`、`show version` 等明确只读 | 请求批准：单击执行；替我审批/受控完全执行：在固定单目标内可自动执行 |
| 中 | 创建文件、安装依赖、修改非关键配置、写入书签 | 请求批准/替我审批：明确批准；受控完全执行：在明确会话范围内可自动执行 |
| 高 | 删除/覆盖、权限修改、服务重启、网络配置、SFTP 删除 | 三种模式均始终人工确认；授权档位不能覆盖高风险规则 |
...
- 建立“请求批准、替我审批、受控完全执行”三档会话策略；第一版默认请求批准。所有档位共享严重动作拒绝、始终确认、目标锁定和组织策略。
...
- 在用户显式选择“替我审批”的会话中允许明确低风险只读自动执行；“受控完全执行”允许更高自治，但不得覆盖硬阻断、始终确认和授权范围。
...
- 三入口一致性矩阵：Agent、Ask、MCP × 三种授权档位 × 低/中/高/严重 × 前台/后台。
...
1. 授权模式已于 2026-08-02 决定：第一版提供三档会话级策略，默认“请求批准”；用户可显式选择“替我审批”或“受控完全执行”。详细规则见 `docs/decisions/002-ai-execution-gateway.md` 与 `docs/research/ai-approval-modes-study.md`。
...
> **先建立统一 Execution Gateway，并让内置 Agent、Ask 代码块、MCP 前台命令和 MCP 后台命令全部经过它；在网关中实现三档会话级授权，再交付能够显示策略结果的单命令卡片。**
